...
*Rule: UI primitives must live in `src/components/ui` (imported via `@/components/ui`) and use design tokens in `src/theme` (no hardcoded colors/spacing).*
...
*Rule: Sensitive writes (points, status, rewards) must happen ONLY via SECURITY DEFINER RPCs (migration `0002_functions.sql`) and never from the client directly.*
...
*Rule: Maps must be imported from `@/components/PlatformMap` (native map + web placeholder) and never `react-native-maps` directly.*
...
*Rule: For dialogs, use `@/lib/dialog` (`confirmAsync`, `notify`, `choosePhotoSource`) rather than React Native's `Alert` (which no-ops on web).*
...
*Rule: app.json configuration must not contain deprecated fields (`splash`, `newArchEnabled`, `android.edgeToEdgeEnabled`).*
...
* **Step-by-Step Proposed Fix**:
  1. Apply Zod's `.trim()` validator to the email fields *before* running the `.email()` check.
  ```typescript
  const schema = z.object({
